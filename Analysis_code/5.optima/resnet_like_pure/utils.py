...
def prepare_dataset(region, data_sample='pure', target='multi', fold=3):
    ...
    # 범주형 & 연속형 변수 분리
    original_categorical_cols = X_train.select_dtypes(include=['object', 'category', 'int64']).columns
    numerical_cols = X_train.select_dtypes(include=['float64']).columns

    # 범주형 변수 One-Hot Encoding (ResNetLike는 임베딩 레이어가 없으므로)
    if len(original_categorical_cols) > 0:
        ...
        # One-Hot 인코딩된 컬럼명으로 업데이트
        categorical_cols = ohe.get_feature_names_out(original_categorical_cols)
    else:
        categorical_cols = []
...
def prepare_dataloader(region, data_sample='pure', target='multi', fold=3, random_state=None):
    ...
    X_train_cat = torch.tensor(X_train[categorical_cols].values, dtype=torch.float32)  # One-Hot은 float32

    X_val_num = torch.tensor(X_val[numerical_cols].values, dtype=torch.float32)
    X_val_cat = torch.tensor(X_val[categorical_cols].values, dtype=torch.float32)  # One-Hot은 float32

    X_test_num = torch.tensor(X_test[numerical_cols].values, dtype=torch.float32)
    X_test_cat = torch.tensor(X_test[categorical_cols].values, dtype=torch.float32)  # One-Hot은 float32
