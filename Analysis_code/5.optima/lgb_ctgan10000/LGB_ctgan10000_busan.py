...
df_ctgan_busan_1 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/ctgan10000/ctgan10000_1_busan.csv"))
df_ctgan_busan_2 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/ctgan10000/ctgan10000_2_busan.csv"))
df_ctgan_busan_3 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/ctgan10000/ctgan10000_3_busan.csv"))

# 데이터 전처리
print("데이터 전처리 중...")
df_ctgan_busan_1 = preprocessing(df_ctgan_busan_1)
df_ctgan_busan_2 = preprocessing(df_ctgan_busan_2)
df_ctgan_busan_3 = preprocessing(df_ctgan_busan_3)
df_busan = preprocessing(df_busan)

# CTGAN 데이터 리스트 (fold 순서와 일치)
df_ctgan_list = [df_ctgan_busan_1, df_ctgan_busan_2, df_ctgan_busan_3]
...
def objective_func(search_space):
    ...
    for df_ctgan, (train_years, val_year) in zip(df_ctgan_list, FOLD_CONFIGS):
        X_train, X_val, y_train, y_val = split_data(
            df_ctgan, df_busan, train_years, val_year
        )
...
# 모델 저장
print("모델 저장 중...")
model_save_path = os.path.join(base_dir, "save_model/lgb_optima/lgb_ctgan10000_busan.pkl")
