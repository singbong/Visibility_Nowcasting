...
START_TIME=$(date +%s)
echo "=========================================="
echo "ResNet-Like Pure 파일 실행 시작"
...
# 실행할 파일 목록
FILES=(
    "resnet_like_pure_busan.py"
    "resnet_like_pure_daegu.py"
    "resnet_like_pure_daejeon.py"
    "resnet_like_pure_gwangju.py"
    "resnet_like_pure_incheon.py"
    "resnet_like_pure_seoul.py"
)
...
for file in "${FILES[@]}"; do
    filepath="resnet_like_pure/$file"
    if [ ! -f "$filepath" ]; then
...
echo "=========================================="
echo "ResNet-Like Pure 파일 실행 완료"
