...
START_TIME=$(date +%s)
echo "=========================================="
echo "ResNet-Like SMOTE 파일 실행 시작"
...
# 실행할 파일 목록
FILES=(
    "resnet_like_smote_busan.py"
    "resnet_like_smote_daegu.py"
    "resnet_like_smote_daejeon.py"
    "resnet_like_smote_gwangju.py"
    "resnet_like_smote_incheon.py"
    "resnet_like_smote_seoul.py"
)
...
for file in "${FILES[@]}"; do
    filepath="resnet_like_smote/$file"
    if [ ! -f "$filepath" ]; then
...
echo "=========================================="
echo "ResNet-Like SMOTE 파일 실행 완료"
