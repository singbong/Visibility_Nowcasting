...
df_busan = pd.read_csv(os.path.join(data_base_dir, "data_for_modeling/busan_train.csv"))
df_smotenc_ctgan_busan_1 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/smotenc_ctgan10000/smotenc_ctgan10000_1_busan.csv"))
df_smotenc_ctgan_busan_2 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/smotenc_ctgan10000/smotenc_ctgan10000_2_busan.csv"))
df_smotenc_ctgan_busan_3 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/smotenc_ctgan10000/smotenc_ctgan10000_3_busan.csv"))

# 데이터 전처리
print("데이터 전처리 중...")
df_smotenc_ctgan_busan_1 = preprocessing(df_smotenc_ctgan_busan_1)
df_smotenc_ctgan_busan_2 = preprocessing(df_smotenc_ctgan_busan_2)
df_smotenc_ctgan_busan_3 = preprocessing(df_smotenc_ctgan_busan_3)
df_busan = preprocessing(df_busan)

# SMOTENC+CTGAN 데이터 리스트 (fold 순서와 일치)
df_smotenc_ctgan_list = [df_smotenc_ctgan_busan_1, df_smotenc_ctgan_busan_2, df_smotenc_ctgan_busan_3]

# 하이퍼파라미터 검색 공간 정의
lgb_search_space = {
    'learning_rate': hp.loguniform('learning_rate', np.log(0.01), np.log(0.2)),
    'max_depth': hp.quniform('max_depth', 3, 15, 1),
    'num_leaves': hp.quniform('num_leaves', 20, 150, 1),  # 2^max_depth 보다는 작게
...
def objective_func(search_space):
    ...
    # 각 fold에 대해 교차 검증 수행
    for df_smotenc_ctgan, (train_years, val_year) in zip(df_smotenc_ctgan_list, FOLD_CONFIGS):
        X_train, X_val, y_train, y_val = split_data(
            df_smotenc_ctgan, df_busan, train_years, val_year
        )
    ...
    # 평균 CSI의 음수값 반환 (hyperopt는 최소화를 수행)
...
print(f"최적화 완료. 최적 파라미터: {lgb_best}")
...
trials_path = os.path.join(base_dir, "optimization_history/lgb_smotenc_ctgan10000_busan_trials.pkl")
...
for fold_idx, (df_smotenc_ctgan, (train_years, val_year)) in enumerate(
    zip(df_smotenc_ctgan_list, FOLD_CONFIGS), start=1
):
...
current_file_dir = os.path.dirname(os.path.abspath(__file__))
base_dir = os.path.dirname(os.path.dirname(current_file_dir))  # 5.optima 디렉토리
os.makedirs(os.path.join(base_dir, "save_model/lgb_optima"), exist_ok=True)
model_save_path = os.path.join(base_dir, "save_model/lgb_optima/lgb_smotenc_ctgan10000_busan.pkl")
