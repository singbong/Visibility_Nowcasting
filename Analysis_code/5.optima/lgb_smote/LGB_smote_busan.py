...
def split_data(df_smote, df_original, train_years, val_year):
    """데이터를 학습용과 검증용으로 분할합니다.
    
    Args:
        df_smote: SMOTE로 오버샘플링된 데이터프레임
    ...
    """
    # 학습 데이터: SMOTE 데이터에서 train_years에 해당하는 데이터
    train_mask = df_smote['year'].isin(train_years)
    X_train = df_smote.loc[train_mask, df_smote.columns != 'multi_class'].copy()
    y_train = df_smote.loc[train_mask, 'multi_class']
...
df_smote_busan_1 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/smote/smote_1_busan.csv"))
df_smote_busan_2 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/smote/smote_2_busan.csv"))
df_smote_busan_3 = pd.read_csv(os.path.join(data_base_dir, "data_oversampled/smote/smote_3_busan.csv"))
...
# SMOTE 데이터 리스트 (fold 순서와 일치)
df_smote_list = [df_smote_busan_1, df_smote_busan_2, df_smote_busan_3]
...
def objective_func(search_space):
    ...
    # 각 fold에 대해 교차 검증 수행
    for df_smote, (train_years, val_year) in zip(df_smote_list, FOLD_CONFIGS):
        X_train, X_val, y_train, y_val = split_data(
            df_smote, df_busan, train_years, val_year
        )
...
for fold_idx, (df_smote, (train_years, val_year)) in enumerate(
    zip(df_smote_list, FOLD_CONFIGS), start=1
):
